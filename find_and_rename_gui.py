...
class App:
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.name_by_path: dict = {}
        self.hit_by_path: dict = {}
        ...
        self.PALETTE = {
            # pastels: status surfaces (listbox rows, hint label, sel summary)
            "scanned":  {"bg": "#e7f3df", "fg": "#2a6d2a"},
            "scanning": {"bg": "#fdf0d9", "fg": "#a35c00"},
            "pending":  {"bg": "#fbe5e5", "fg": "#9b2a2a"},
            "neutral":  {"bg": "#f6f8fa", "fg": "#57606a"},
    ...
    def _build(self) -> None:
        ...
        content_area.grid(row=3, column=0, sticky="nsew", padx=6, pady=2)
        content_area.columnconfigure(0, weight=1)
        content_area.rowconfigure(0, weight=1)

        nb = ttk.Notebook(content_area)
        ...
        self.nb = nb
        nb.bind("<<NotebookTabChanged>>", lambda _e: self._refresh_action_buttons())

        side = ttk.LabelFrame(content_area, text="Selected items")
        side.grid(row=0, column=1, sticky="ns", padx=(6, 0))
        self.sel_label = tk.Label(side, text="Pick rows below", anchor="w", padx=6, pady=4,
                                  bg=self.PALETTE["neutral"]["bg"], fg=self.PALETTE["neutral"]["fg"],
                                  width=24, justify="left")
        self.sel_label.grid(row=0, column=0, sticky="ew", padx=6, pady=(6, 4))
        side_btn_w = 22
        self.open_file_btn = tk.Button(side, text="Open", width=side_btn_w,
                                       relief="raised", bd=1, command=self.on_open_files)
        self.open_file_btn.grid(row=1, column=0, padx=6, pady=2)
        self.open_folder_btn = tk.Button(side, text="Open containing folder", width=side_btn_w,
                                         relief="raised", bd=1, command=self.on_open_folders)
        self.open_folder_btn.grid(row=2, column=0, padx=6, pady=2)
        self.copy_path_btn = tk.Button(side, text="Copy path(s)", width=side_btn_w,
                                       relief="raised", bd=1, command=self.on_copy_paths)
        self.copy_path_btn.grid(row=3, column=0, padx=6, pady=2)
        ttk.Separator(side, orient="horizontal").grid(row=4, column=0, sticky="ew", padx=6, pady=8)
        self.rename_sel_btn = tk.Button(side, text="Rename selected only", width=side_btn_w,
                                        relief="raised", bd=1, command=self.on_rename_selected)
        self.rename_sel_btn.grid(row=5, column=0, padx=6, pady=2)
        self.rewrite_sel_btn = tk.Button(side, text="Rewrite selected only", width=side_btn_w,
                                         relief="raised", bd=1, command=self.on_rewrite_selected)
        self.rewrite_sel_btn.grid(row=6, column=0, padx=6, pady=(2, 8))

        self.tip_label = tk.Label(
            root,
            text="Pick rows in any tab, then use the side panel.  Double-click opens a file.  Right-click also works.",
            fg="#6e7681",
            anchor="w",
            padx=6,
            font=("Segoe UI", 8),
        ...
        self.status_label.grid(row=5, column=0, sticky="ew", padx=6, pady=(2, 0))
        ...
        self.progress.grid(row=6, column=0, sticky="ew", padx=6, pady=(2, 6))

        self.ctx_menu = tk.Menu(root, tearoff=0)
        self.ctx_menu.add_command(label="Open", command=lambda: self._ctx_action("open_file"))
    ...
    def _bind_context_menu(self, tree: ttk.Treeview) -> None:
        ...
        tree.bind("<<TreeviewSelect>>", lambda _e: self._refresh_action_buttons())
    ...
    def on_scan(self) -> None:
        ...
        if not roots:
            ...
            return
        self.scanning_dirs = set(keep_raw)
    ...
    def _populate_after_find(self) -> None:
        self.name_by_path = {str(e.path): e for e in self.name_matches}
        self.hit_by_path = {str(e.path): (e, n) for e, n in self.content_hits}

        tree = self.name_tree
        tree.delete(*tree.get_children())
        for e in sorted(self.name_matches, key=lambda x: x.path.as_posix().lower())[:5000]:
            tree.insert("", "end", iid=str(e.path), values=(e.kind, e.ext, e.stamp, str(e.path)))
        ...
        for entry, n in sorted(self.content_hits, key=lambda x: -x[1])[:5000]:
            total += n
            tree.insert("", "end", iid=str(entry.path),
                        values=(n, entry.ext, entry.encoding or "", str(entry.path)))
    ...
    def _selected_paths_active(self) -> list[str]:
        t = self._active_tree()
        if t is None:
            return []
        return self._selected_paths(t)

    def _refresh_side_panel(self) -> None:
        paths = self._selected_paths_active()
        n = len(paths)
        has_repl = bool(self.replacement_var.get())
        idx = self.nb.index(self.nb.select()) if self.nb.tabs() else -1
        ...
        if n == 0:
            self.sel_label.configure(
                text="Pick rows below",
                bg=self.PALETTE["neutral"]["bg"], fg=self.PALETTE["neutral"]["fg"],
            )
        else:
            n_files = sum(1 for p in paths if os.path.isfile(p))
            ...
            self.sel_label.configure(
                ...
                bg=self.PALETTE["scanned"]["bg"], fg=self.PALETTE["scanned"]["fg"],
            )

        self._apply_btn(self.open_file_btn,   "btn_primary", n > 0 and not self._busy)
        self._apply_btn(self.open_folder_btn, "btn_primary", n > 0 and not self._busy)
        self._apply_btn(self.copy_path_btn,   "btn_primary", n > 0 and not self._busy)

        can_rename = (idx == 0 and n > 0 and has_repl and not self._busy
                      and any(p in self.name_by_path for p in paths))
        can_rewrite = (idx == 1 and n > 0 and has_repl and not self._busy
                       and any(p in self.hit_by_path for p in paths))
        self._apply_btn(self.rename_sel_btn,  "btn_danger", can_rename)
        self._apply_btn(self.rewrite_sel_btn, "btn_danger", can_rewrite)

    def on_open_files(self) -> None:
        t = self._active_tree()
        if t is not None:
            self._open_selected(t, "open_file")

    def on_open_folders(self) -> None:
        t = self._active_tree()
        if t is not None:
            self._open_selected(t, "open_folder")

    def on_copy_paths(self) -> None:
        t = self._active_tree()
        if t is not None:
            self._open_selected(t, "copy_path")

    def on_rename_selected(self) -> None:
        if self._busy or not self.replacement_var.get():
            return
        paths = self._selected_paths_active()
        entries = [self.name_by_path[p] for p in paths if p in self.name_by_path]
        if not entries:
            messagebox.showinfo("find_and_rename",
                                "Select rows in the Name matches tab to rename specific items.")
            return
        ...
        if repl == self.last_phrase:
            ...
            return
        plans, rejected = plan_renames_ex(entries, self.last_phrase, repl, self.last_opts)
        self._handle_rejected(rejected)
        if not plans:
            messagebox.showinfo("find_and_rename",
                                "No valid renames produced. Check the Errors tab if any names were rejected.")
            return
        preview = "\n".join(f"{s.name}  ->  {d.name}" for s, d in plans[:30])
        more = f"\n... {len(plans) - 30} more" if len(plans) > 30 else ""
        ok = messagebox.askyesno(
            "Confirm rename (selected only)",
            f"Rename {len(plans)} selected item(s)?\n\nMode: {self.last_opts.label}\n\n"
            f"{preview}{more}",
        ...
        if not ok:
            return
        self._set_busy(True, "Renaming selected items...")
        self._progress_indeterminate()
        self.log_line(f"--- rename (selected) {len(plans)} item(s)  ({self.last_opts.label})")
        log_to_file(f"Rename (selected) start: '{self.last_phrase}' -> '{repl}'  "
                    f"({self.last_opts.label}), {len(plans)} item(s)")
    ...
    def on_rewrite_selected(self) -> None:
        if self._busy or not self.replacement_var.get():
            return
        paths = self._selected_paths_active()
        hits = [self.hit_by_path[p] for p in paths if p in self.hit_by_path]
        if not hits:
            messagebox.showinfo("find_and_rename",
                                "Select rows in the Content matches tab to rewrite specific files.")
            return
        repl = self.replacement_var.get()
        if repl == self.last_phrase:
            messagebox.showinfo("find_and_rename", "Replacement equals the search phrase.")
            return
        occ = sum(c for _, c in hits)
        ok = messagebox.askyesno(
            "Confirm content rewrite (selected only)",
            f"Rewrite '{self.last_phrase}' -> '{repl}' inside {len(hits)} selected file(s) "
            f"({occ} occurrence(s))?\n\nMode: {self.last_opts.label}\n\n"
            f"This modifies files in place and cannot be undone by this tool.",
            default=messagebox.NO,
            icon=messagebox.WARNING,
        )
        if not ok:
            return
        self._set_busy(True, "Rewriting selected files...")
        self._progress_indeterminate()
        self.log_line(f"--- rewrite (selected) '{self.last_phrase}' -> '{repl}'  ({self.last_opts.label})")
        log_to_file(f"Rewrite (selected) start: '{self.last_phrase}' -> '{repl}'  "
                    f"({self.last_opts.label}), {len(hits)} file(s)")
    ...
    def _refresh_dir_status(self) -> None:
        ...
        self._apply_btn(self.scan_btn, "btn_primary", scan_enabled)
    ...
    def _handle_rejected(self, rejected: list) -> None:
        if not rejected:
            return
        for p, msg in rejected:
            self.op_errors.append(("rename-rejected", str(p), msg))
            self.log_line(f"  rename rejected: {p}  --  {msg}")
            log_to_file(f"  rename rejected: {p}  --  {msg}")
        self._append_op_errors()
        sample = rejected[0][1]
        messagebox.showwarning(
            "Some names could not be produced",
            f"{len(rejected)} item(s) were skipped because the replacement would create "
            f"an invalid filename.\nExample: {sample}\n\nSee the Errors tab for the full list.",
        )
    ...
    def on_rename(self) -> None:
        ...
        if repl == self.last_phrase:
            messagebox.showinfo("find_and_rename", "Replacement equals the search phrase.")
            return
        plans, rejected = plan_renames_ex(self.name_matches, self.last_phrase, repl, self.last_opts)
        self._handle_rejected(rejected)
        if not plans:
            messagebox.showinfo("find_and_rename",
                                "No valid renames produced. Check the Errors tab if any names were rejected.")
