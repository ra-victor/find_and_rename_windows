...
def _is_invalid_basename(name: str) -> str:
    if not name:
        return "empty name"
    if name in (".", ".."):
    ...
    for ch in _BAD_NAME_CHARS:
        if ch in name:
            return f"invalid character {ch!r} in name"
    return ""
...
def plan_renames(
    ...
    plans, _ = plan_renames_ex(matches, phrase, replacement, opts)
    return plans
...
def plan_renames_ex(
    ...
    """Same as plan_renames, but also returns a list of (path, reason) rejects."""
    ...
    for e in ordered:
        ...
        if n == 0 or new_name == e.path.name:
            continue
        reason = _is_invalid_basename(new_name)
        if reason:
            rejected.append((e.path, reason))
            continue
        try:
            dst = e.path.with_name(new_name)
        except ValueError as ve:
            rejected.append((e.path, str(ve)))
            continue
        plans.append((e.path, dst))
